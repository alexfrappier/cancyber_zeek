...
export {
	...
	const cancyber_per_item_expiration = -1min &redef;

	redef record MetaData += {
		cancyber_expire: interval &default = cancyber_per_item_expiration;

		## Internal value tracks time of item creation, last update.
		cancyber_last_update: time  &default = network_time();

		cancyber_hits:        int      &default = 0;
		cancyber_dns_hits:        int      &default = 0;
		cancyber_scan_hits:        int      &default = 0;
...
hook extend_match(info: Info, s: Seen, items: set[Item])
{
	...
	for ( item in items )
	{
		...
		if (meta$source != "CanCyber") {
			next;
		}

		if (meta$cancyber_expire > 0 sec && meta$cancyber_last_update + meta$cancyber_expire < network_time() )
		{
		...
		if (s?$where) {
			hit += fmt("|msg: Intel hit on %s at %s", s$indicator, s$where);
		}
		#hit += fmt(" [%s]",meta$desc);
		#hit += fmt(" (%s)",meta$url);
